            ...
            <div key={r.id} className="p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-500 mt-0.5">
                      <FontAwesomeIcon icon={faCar} className="mr-1 text-slate-400" />
                      {r.vehicle_brand} {r.vehicle_color ? `· ${r.vehicle_color}` : ""}
                    </div>
                  </div>
                </div>
                <span
                  className={`py-1 px-2.5 rounded-full text-xs font-semibold border ${
                    r.urgency_level === "Urgent"
                      ? "bg-red-50 border-red-200 text-red-600"
                      : "bg-[#ECFDF3] border-[#BBF7D0] text-green-700"
                  }`}
              ...
              <div className="bg-slate-50 p-3 rounded-xl flex flex-col gap-1.5 text-xs">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-slate-900">{r.issue_category}</span>
                </div>
                {activeTab === "new" && Number(r.requires_tow) === 1 && (
                  <div className="text-red-600 font-semibold flex items-center gap-1 mt-0.5">
                    <FontAwesomeIcon icon={faTruck} /> Tow Truck Required
                    {r.pickup_landmark && <span className="text-slate-500 font-normal">({r.pickup_landmark})</span>}
                  </div>
                )}
                {activeTab !== "new" && (
                  <div className="flex flex-col gap-0.5 mt-1 border-t border-slate-200/60 pt-1.5">
                    ...
                    <span className="text-slate-700 font-medium">{r.cancellation_reason || "No reason provided"}</span>
                  </div>
                )}
              </div>
              ...
              {activeTab === "new" && (
                <div className="flex flex-col sm:flex-row gap-2 pt-1">
                  <button
                    onClick={() => setSelectedRequest(r)}
                    className="w-full sm:flex-1 py-2 px-3 rounded-lg border border-green-700 bg-white text-green-700 font-bold text-xs cursor-pointer text-center"
                  >
                    View Details
                  </button>
                  {r.status === "Pending" ? (
                    <div className="flex gap-2 w-full sm:flex-1">
                      <button
                        className="flex-1 py-2 px-3 rounded-lg border-none bg-green-700 text-white font-bold text-xs cursor-pointer text-center"
                        onClick={() => handleAcceptClick(r)}
                      >
                        Accept
                      </button>
                      <button
                        className="flex-1 py-2 px-3 rounded-lg border border-red-200 text-red-600 bg-white font-bold text-xs cursor-pointer text-center"
                        onClick={() => handleDeclineClick(r)}
                      >
                        Decline
                      </button>
                    ...
                    <span className="w-full text-center py-2 px-3 rounded-lg bg-amber-100 text-amber-800 font-semibold text-xs">
                      Waiting for customer confirmation
                    </span>
                  ) : (
                    <span className="w-full text-center py-2 px-3 rounded-lg bg-red-100 text-red-600 font-semibold text-xs">
                      {r.status}
                    </span>
                  )}
                </div>
